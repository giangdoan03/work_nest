    ...
    <div>
        <a-card bordered>
            <a-page-header
                ...
                :sub-title="contract?.title || contract?.name || '—'"
                @back="() => router.back()"
...
import { getContractAPI, getContractStepCountAPI } from '@/api/contract.js'
import { deleteTask, updateTask, getTasksByContractStep } from '@/api/task'
...
const userOptions = computed(() => users.value.map(u => ({ label: u.name, value: Number(u.id) })))
...
const buildTaskLink = (row) => {
    if (contractId && stepId) {
        return {
            name: 'contract-task-info-in-step',
            params: { contractId: String(contractId), stepId: String(stepId), id: String(row.id) },
        }
    }
    return { name: 'contract-task-info', params: { id: String(row.id) } }
}
...
const reloadTasks = async () => {
    const res = await getTasksByContractStep(stepId, {})
...
const openSubtaskDrawer = (parentRow) => {
    subDrawerParent.value = {
        id: parentRow.id,
        linked_type: 'contract',                 // ✅ đúng context
...
const approveTask = async (row) => {
    ...
    if (!ok) return
    try {
        await approveTaskSimpleAPI(row.id, {})
...
const rejectTask = async (row) => {
    ...
    if (!ok) return
    try {
        await rejectTaskSimpleAPI(row.id, {})
